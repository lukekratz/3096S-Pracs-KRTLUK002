...
static uint16_t addressNumber = 0x00;

static uint16_t PSC_val = 7999;
static uint8_t valueX;
...
int main(void)
{
  ...
  write_to_address(0x01, patterns[1]);
  write_to_address(0x02, patterns[2]);
  write_to_address(0x03, patterns[3]);
  write_to_address(0x04, patterns[4]);
  write_to_address(0x05, patterns[5]);

  //valueX = read_from_address(0x05);
  ...
  while (1)
  {
	...
	if (PSC_val == 7999){
		__HAL_TIM_SET_PRESCALER(&htim16, 3999);
		PSC_val = 3999;
	}
	else if ((HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == 0) && PSC_val == 3999){
		__HAL_TIM_SET_PRESCALER(&htim16, 7999);
		PSC_val = 7999;
	}
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t readValue = 0;
	// Evaluate if the temporary address for reading from EEPROM is greater
	// than 0x05. If this is the case then the tempAddress variable must be
	// reset to 0x00.
	if (addressNumber > 0x05){
		addressNumber = 0x00;
	}

	// Read value stored in EEPROM
	readValue = read_from_address(addressNumber);

	// Check for bit errors. If there is an error, the value 0b00000001 will
	// be output to the LEDs to indicate an SPI failure.
	if (readValue != patterns[addressNumber]){
		GPIOB -> ODR = 0b00000001;
	}

	// If the data is correct, then write the binary pattern to the LEDs
	else if (readValue == patterns[addressNumber]){
		GPIOB -> ODR = readValue;
	}

	// Increment the tempAddress variable by 1
